...
const ImgZoom = forwardRef(({imgSrc, alt, otherClass = ""}, ref) => {
    ...
    return(
        ...
        <div ref={ref} className={"group w-full relative duration-200 hover:border-8 border-white cursor-zoom-in "+otherClass} onClick={handleClic}>
            <img src={import.meta.env.BASE_URL + imgSrc.replace(/^\/+/, "")} alt={alt +" miniatura"} loading="lazy" decoding="async" className="w-full h-full object-cover"/>
        ...
        </div>
        {
            showFullImage &&
                ...
                <div className="fixed w-screen h-screen inset-0 z-[100] bg-[rgba(18,3,20,0.9)] cursor-zoom-out justify-center items-center" onClick={handleClose}>
                    <img ref={imgRef} src={import.meta.env.BASE_URL + imgSrc.replace(/^\/+/, "")} loading="lazy" decoding="async" alt={alt +" expandida"} className="absolute object-cover rounded-xl"/> 
                </div>
                ,document.body)
        }
        </>
