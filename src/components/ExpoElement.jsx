        ...
        <div ref={ref} className="fixed left-0 top-[100vh] duration-100 w-full h-screen bg-[rgb(18,3,20)] z-50 flex items-end justify-center ">
            ...
            ">
                <div className="h-9/12
                    md:h-10/12
                ">
                    {!isVideo && <img className="bg-black w-full object-cover h-full" src={import.meta.env.BASE_URL + urlImage.replace(/^\/+/, "")} alt={`${nameElement} image`} />}
                    {isVideo && <video className="w-full object-cover h-full bg-black" src={import.meta.env.BASE_URL + urlVideo.replace(/^\/+/, "")} controls/> }
                </div>
                ...
                ">
                    ...
                    "> {nameElement} </span> <span className="text-base text-gray-500
                        md:text-lg
                    "> {sizes} </span></div>
                     <a className="px-4 py-2 rounded-full bg-white text-black flex items-center font-bold text-center justify-center"
                        href={isVideo ? urlVideo : urlImage}
                        download={isVideo ? "video" : "imagen"}
                        >
